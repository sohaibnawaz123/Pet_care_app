...
import 'package:pet_care_app/core/network/socket/socket_status.dart';
...
class SocketManager {
  ...
  Future<Either<NetworkFailure, bool>> connect({
    ...
    try {
      ...
      _socket!.onConnect((_) {
        Utils.logInfo('Socket connected');
      ...
      _socket!.onDisconnect((_) {
        Utils.logInfo('Socket disconnected');
        _connectionStateController.add(SocketStatus.disconnected);
      });

      _socket!.onReconnect((_) {
        Utils.logInfo('Socket reconnected');
        _connectionStateController.add(SocketStatus.connected);
        _reattachEventListeners();
      });

      _socket!.onError((error) {
        Utils.logInfo('Socket error: $error');
        _connectionStateController.add(SocketStatus.error);
      });

      _socket!.onConnectError((error) {
        Utils.logInfo('Socket connection error: $error');
        _connectionStateController.add(SocketStatus.error);
        if (!completer.isCompleted) {
          completer.complete(
  ...
  void _attachListener(String event) {
    ...
    Utils.logInfo('Attached listener for event: $event');
  }
  ...
  void _detachListener(String event) {
    ...
    if (!controller.hasListener) {
      ...
      Utils.logInfo('Detached listener for event: $event');
    }
  }
  ...
  void _reattachEventListeners() {
    ...
    Utils.logInfo('Reattached ${eventsToReattach.length} event listeners');
  }
